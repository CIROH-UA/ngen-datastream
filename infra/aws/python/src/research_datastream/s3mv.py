...
class MoveStats:
    ...
    def add_sample_move(
        ...
        with self.lock:
            ...
            if has_pattern:
                self.pattern_sample_moves.append(
...
def should_process_object(
    key: str, contains_pattern: Optional[str], ignore_pattern: Optional[str]
...
def move_object(
    ...
    if dry_run:
        ...
        if stats:
            ...
            stats.add_sample_move(
                source_bucket, source_key, dest_bucket, dest_key, has_pattern
            )
        return True, None
    ...
    for attempt in range(max_retries):
        try:
            ...
            if (
                error_code in ["SlowDown", "ServiceUnavailable", "RequestTimeout"]
...
def process_object(args: Tuple, stats: MoveStats, progress_interval: int) -> None:
    """
    ...
    if not should_process:
        ...
        if should_print:
            ...
            rate = stats.get_rate()
            print(
                f"Progress: {total:,} processed | {moved:,} moved | {skipped:,} skipped | {errors:,} errors | Rate: {rate:.1f} files/sec"
            )
        return
...
Examples:
  ...
  %(prog)s --source_bucket my-bucket --prefix_old v2.2 --prefix_new v2.2_new \\
    ...
    parser.add_argument(
        "--prefix_old", required=True, help="Current prefix path in source bucket"
    )
    parser.add_argument(
        "--prefix_new", required=True, help="New prefix path in destination bucket"
    )
    parser.add_argument(
        "--ignore_pattern",
        default=None,
    ...
    parser.add_argument(
        "--profile", default=None, help="AWS profile name to use (optional)"
    )
    ...
    if args.delete_pattern and (args.replace_pattern or args.with_pattern):
        print(
            "Error: --delete_pattern cannot be used with --replace_pattern or --with_pattern"
        )
        sys.exit(1)
    ...
    if args.replace_pattern:
        if args.with_pattern:
            print(
                f"Replace Pattern:  '{args.replace_pattern}' -> '{args.with_pattern}'"
            )
        else:
    ...
    if args.skip > 0:
        if args.skip >= len(objects):
            print(
                f"Skip value ({args.skip}) is >= total objects ({len(objects)}). Nothing to do."
            )
            return
        ...
        if first_samples or last_samples:
            ...
            if args.replace_pattern:
                ...
                if pattern_affected == 0:
                    print(
                        f"⚠️  WARNING: No files contain the pattern '{args.replace_pattern}'"
                    )
                    print(
                        f"   The pattern will have no effect. Check your pattern spelling."
                    )
                else:
                    ...
                    if pattern_samples:
                        ...
                        for i, (src_bucket, src_key, dst_bucket, dst_key) in enumerate(
                            pattern_samples, 1
                        ):
                            print(f"\n  {i}. Source:")
            ...
            if last_samples and len(first_samples) + len(last_samples) > len(
                first_samples
            ):
                print(
                ...
                for i, (src_bucket, src_key, dst_bucket, dst_key) in enumerate(
                    last_samples, moved - len(last_samples) + 1
                ):
                    print(f"\n  {i}. Source:")
                    print(f"     s3://{src_bucket}/{src_key}")
                    print(f"     Destination:")
