...
TIMEOUTS = {"short_range": 3600, "medium_range": 7200, "analysis_assim_extend": 3600}
...
def edit_dict(
    ...
    exec_jvpu["instance_parameters"]["BlockDeviceMappings"][0]["Ebs"]["VolumeSize"] = (
        volume_size
    )
    instance_name = copy.deepcopy(
        exec_jvpu["instance_parameters"]["TagSpecifications"][0]["Tags"][0]["Value"]
    )
    exec_jvpu["instance_parameters"]["TagSpecifications"][0]["Tags"][0]["Value"] = (
        re.sub(PATTERN_VPU, jvpu, instance_name)
    )
    instance_name = copy.deepcopy(
        exec_jvpu["instance_parameters"]["TagSpecifications"][0]["Tags"][0]["Value"]
    )
    exec_jvpu["instance_parameters"]["TagSpecifications"][0]["Tags"][0]["Value"] = (
        re.sub(PATTERN_RUN_TYPE_L, run_type.lower(), instance_name)
    )
    ...
    for j, jcmd in enumerate(cmds):
        ...
        stream_command = copy.deepcopy(cmds[j])
        cmds[j] = re.sub(
            PATTERN_FCST, PATTERN_FORECAST[run_type.lower()], stream_command
        )
        stream_command = copy.deepcopy(cmds[j])
    ...
    if "datastream_command_options" in exec_jvpu.keys():
        exec_jvpu[key] = dict(
            [[key, value] for key, value in zip(exec_jvpu[key].keys(), cmds)]
        )
    else:
        exec_jvpu[key] = cmds
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="A statemachine execution json file for ngen-datastream forcingprocessor",
    )
    parser.add_argument(
        "--exec_template_vpu",
        type=str,
        help="A statemachine execution json file for ngen-datastream",
    )
    parser.add_argument(
        "--ami_file",
        type=str,
        help="Text file that holds the AMIs for each architecture",
    )
    parser.add_argument("--arch", type=str, help="x86 or arm")
    parser.add_argument(
        "--inputs", type=str, help="json that holds inputs for each run type"
    )
    parser.add_argument(
        "--out_dir", type=str, help="Path to write executions out to, must not exist"
    )
