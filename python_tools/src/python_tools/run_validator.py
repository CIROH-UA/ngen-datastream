...
def check_forcings(forcings_start, forcings_end, n, start_time, end_time, dt_s):
...
def validate_catchment_files_worker(args):
    validations, catchments, forcing_dir, realization_times = args
    start_time, end_time, dt_s = realization_times
        ...
        if jval == "forcing":
            if files[0].endswith(".nc"):
                ...
                with xr.open_dataset(os.path.join(forcing_dir, nc_file)) as ngen_forcings:
                    ...
                    check_forcings(forcings_start, forcings_end, len(ngen_forcings.time.values), start_time, end_time, dt_s)
                    continue
        ...
        for j, jcatch in enumerate(catchments):
            ...
            if jval == "forcing":
                if j == 0:
                    ...
                    check_forcings(forcings_start, forcings_end, len(df['time']), start_time, end_time, dt_s)
...
def validate_data_dir(data_dir):
    ...
    jdir_dict = {"CFE": "CFE", "PET": "PET", "NoahOWP": "NOAH-OWP-M"}
    ...
    for jform in serialized_realization.global_config.formulations:
        for jmod in jform.params.modules:
            ...
            validate_files[jmod.params.model_name] = {
                "pattern": pattern,
                "files": sorted([x for x in config_files if bool(compiled.match(x))])
            }
    ...
    worker_args = [
        (val_dict_list[idx], catchment_list_list[idx], forcing_dir, realization_times)
        for idx in range(nprocs)
    ]

    with cf.ProcessPoolExecutor() as pool:
        pool.map(validate_catchment_files_worker, worker_args)
